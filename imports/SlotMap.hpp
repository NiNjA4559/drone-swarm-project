...
#ifndef SLOT_MAP_ASSERT
#    include <cassert>
#    define SLOT_MAP_ASSERT(Expr) assert(Expr)
#endif

#ifndef SLOT_MAP_ASSERT_NOEXCEPT
#    define SLOT_MAP_ASSERT_NOEXCEPT (true)
#endif

#ifndef SLOT_MAP_PAGE_SIZE
#    define SLOT_MAP_PAGE_SIZE 0xffff
#endif

#ifndef SLOT_MAP_MIN_SLOT_NUM
#    define SLOT_MAP_MIN_SLOT_NUM 32
#endif

#ifndef SLOT_MAP_MAX_ROOT_WORD_NUM
#    define SLOT_MAP_MAX_ROOT_WORD_NUM 8
#endif

#if defined(_MSC_VER)
#    define SLOT_MAP_INLINE __forceinline
#elif defined(__GNUC__) || defined(__clang__)
#    define SLOT_MAP_INLINE inline __attribute__((always_inline))
#else
#    define SLOT_MAP_INLINE inline
#endif
...
namespace SlotMap
{
    namespace detail
    {
        ...
        template <typename value_t, size_t size_v, size_t root_size_v = SLOT_MAP_MAX_ROOT_WORD_NUM>
        ...
        struct hierarchical_bitset
        {
            ...
            [[nodiscard]] constexpr bool is_set(const size_t index) const noexcept(SLOT_MAP_ASSERT_NOEXCEPT)
            {
                SLOT_MAP_ASSERT(index < size_v);
                return traits_type::is_set(bits, index);
            }

            [[nodiscard]] constexpr bool is_unset(const size_t index) const noexcept(SLOT_MAP_ASSERT_NOEXCEPT)
            {
                SLOT_MAP_ASSERT(index < size_v);
                return not traits_type::is_set(bits, index);
            }

            constexpr void reset(const size_t index) noexcept(SLOT_MAP_ASSERT_NOEXCEPT)
            {
                SLOT_MAP_ASSERT(index < size_v);
    ...
    struct slot_value
    {
        ...
        SLOT_MAP_INLINE void construct(args_t&&... args) noexcept(std::is_nothrow_constructible_v<value_t, args_t&&...>)
        {
            value_t* ptr = reinterpret_cast<value_t*>(std::addressof(bytes));
            std::construct_at(ptr, std::forward<args_t>(args)...);
        }

        SLOT_MAP_INLINE void destroy() noexcept(std::is_nothrow_destructible_v<value_t>)
        {
            value_t* ptr = reinterpret_cast<value_t*>(std::addressof(bytes));
            std::destroy_at(ptr);
        }

        SLOT_MAP_INLINE value_t& get() noexcept
        {
            value_t* ptr = reinterpret_cast<value_t*>(std::addressof(bytes));
            return *ptr;
        }
    ...
    struct slot_storage_static
    {
        ...
        SLOT_MAP_INLINE constexpr std::tuple<slot_value<value_t>&, version_t&> at(const size_t index) noexcept(SLOT_MAP_ASSERT_NOEXCEPT)
        {
            SLOT_MAP_ASSERT(index < size_v);
        ...
        SLOT_MAP_INLINE constexpr std::tuple<slot_value<value_t>*, version_t*> try_at(const size_t index) noexcept
        {
    ...
    struct slot_storage_dynamic
    {
        ...
        static constexpr size_t page_size = SLOT_MAP_PAGE_SIZE;
        static constexpr size_t num_slot_min = SLOT_MAP_MIN_SLOT_NUM;
        ...
        SLOT_MAP_INLINE constexpr std::tuple<slot_value<value_t>&, version_t&> at(const size_t index) noexcept
        {
        ...
        SLOT_MAP_INLINE constexpr std::tuple<slot_value<value_t>*, version_t*> try_at(const size_t index) const noexcept
        {
            const size_t block_index = index / num_slot;
            const size_t slot_index = index % num_slot;
      ...
      private:
        ...
        [[nodiscard]] SLOT_MAP_INLINE constexpr block& get_block(const size_t index) noexcept(SLOT_MAP_ASSERT_NOEXCEPT)
        {
            SLOT_MAP_ASSERT(index < num_block);
        ...
        [[nodiscard]] SLOT_MAP_INLINE constexpr block* try_get_block(const size_t index) const noexcept(SLOT_MAP_ASSERT_NOEXCEPT)
        {
            SLOT_MAP_ASSERT(index < num_block);
    ...
    struct basic_slot_map
    {
        ...
        struct iterator_impl
        {
            ...
            constexpr std::tuple<key_t, value_type&> operator*() const noexcept(SLOT_MAP_ASSERT_NOEXCEPT)
            {
                SLOT_MAP_ASSERT(_storage != nullptr);
        ...
        constexpr ~basic_slot_map() noexcept(std::is_nothrow_destructible_v<value_t> and SLOT_MAP_ASSERT_NOEXCEPT)
        {
            SLOT_MAP_ASSERT(_data != nullptr);
        ...
        [[nodiscard]] constexpr iterator begin() noexcept(SLOT_MAP_ASSERT_NOEXCEPT)
        {
            SLOT_MAP_ASSERT(_data != nullptr);
            return iterator { _data->storage, _data->bitset.as_view().begin() };
        }

        [[nodiscard]] constexpr iterator end() noexcept(SLOT_MAP_ASSERT_NOEXCEPT)
        {
            SLOT_MAP_ASSERT(_data != nullptr);
            return iterator { _data->storage, _data->bitset.as_view().end() };
        }

        [[nodiscard]] constexpr const_iterator begin() const noexcept(SLOT_MAP_ASSERT_NOEXCEPT)
        {
            SLOT_MAP_ASSERT(_data != nullptr);
            return const_iterator { _data->storage, _data->bitset.as_view().begin() };
        }

        [[nodiscard]] constexpr const_iterator end() const noexcept(SLOT_MAP_ASSERT_NOEXCEPT)
        {
            SLOT_MAP_ASSERT(_data != nullptr);
        ...
        template <typename... args_t>
        [[nodiscard]] constexpr std::optional<key_t> try_emplace(args_t&&... args) noexcept(std::is_nothrow_constructible_v<value_t, args_t&&...> and SLOT_MAP_ASSERT_NOEXCEPT)
        {
            SLOT_MAP_ASSERT(_data != nullptr);
        ...
        template <typename... args_t>
        [[nodiscard]] constexpr key_t emplace(args_t&&... args) noexcept(std::is_nothrow_constructible_v<value_t, args_t&&...> and SLOT_MAP_ASSERT_NOEXCEPT)
        {
            const std::optional<key_t> key = try_emplace(std::forward<args_t>(args)...);
            SLOT_MAP_ASSERT(key.has_value());
            return key.value();
        }

        [[nodiscard]] constexpr value_t* try_at(const key_t& key) noexcept(SLOT_MAP_ASSERT_NOEXCEPT)
        {
            SLOT_MAP_ASSERT(_data != nullptr);
        ...
        [[nodiscard]] constexpr const value_t* try_at(const key_t& key) const noexcept(SLOT_MAP_ASSERT_NOEXCEPT)
        {
            return const_cast<basic_slot_map&>(*this).try_at(key);
        }

        [[nodiscard]] constexpr value_t& at(const key_t& key) noexcept(SLOT_MAP_ASSERT_NOEXCEPT)
        {
            SLOT_MAP_ASSERT(_data != nullptr);

            const index_type index = key_traits_t::index(key);

            SLOT_MAP_ASSERT(_data->bitset.is_set(index));
            ...
            SLOT_MAP_ASSERT(key_traits_t::version(key) == slot_version);

            return slot.get();
        }

        [[nodiscard]] constexpr const value_t& at(const key_t& key) const noexcept(SLOT_MAP_ASSERT_NOEXCEPT)
        {
            return const_cast<basic_slot_map&>(*this).at(key);
        }

        constexpr bool erase(const key_t& key) noexcept(std::is_nothrow_destructible_v<value_t> and SLOT_MAP_ASSERT_NOEXCEPT)
        {
            SLOT_MAP_ASSERT(_data != nullptr);
